...
```{r}
convert_thinner_oct <- df_thinner_4 %>% filter(date >= as_datetime("2018-07-01")) %>%    
    ...
    filter(date >= as_datetime("2018-07-01"))
```
...
columns_to_mode <- c("channelGrouping", "country", "city", "subContinent", 
                     "deviceCategory", "operatingSystem", "browser")



df_mode_oct <-  convert_thinner_oct %>%  
    group_by(fullVisitorId) %>% summarise_at(.vars =columns_to_mode,  find_mode) 


df_mode_oct_action <-  convert_thinner_oct %>%    
...
df_til_oct <- convert_thinner_oct %>% 
  group_by(fullVisitorId) %>%
  dplyr::summarise(
    ...
    dates_oct = list(date),
    session_month_n=sum(map_lgl(date, has_session_within_dates, date_start="2018-09-15", date_end="2018-10-15"  ))>0,
    session_month_nm1=sum(map_lgl(date, has_session_within_dates, date_start="2018-08-15", date_end="2018-09-14"  ))>0,
    session_month_nm2=sum(map_lgl(date, has_session_within_dates, date_start="2018-07-15", date_end="2018-08-14"  ))>0,
...
ids_unique <- df_thinner_4 %>% dplyr::count(fullVisitorId) %>% select(fullVisitorId)
...
write_rds(final_df_new, "data-files/final_df_new_converters.rds")
```
